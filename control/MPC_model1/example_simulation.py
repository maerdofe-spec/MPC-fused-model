"""Hardware-free demonstration of the model-1-only MPC."""
...
def main() -> None:
    ...
    for step in range(60):
        result = controller.solve(state, tau_previous)
        # Use the same model-1 plant in this consistency demonstration.
        state = (
            model.A_d @ state
            + model.B_d @ (result.force - tau_previous)
        )
        tau_previous = result.force.copy()
        if step % 10 == 0 or step == 59:
            print(f"{step:4d} | {state[:3]} | {result.force} | {result.status}")

    print("reference position:", controller.config.reference_position)
